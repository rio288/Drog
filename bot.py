...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # معالجة إضافة مفتاح اشتراك (للمشرفين فقط)
    ...
    if text == "🎬 تجهيز البث":
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("تفعيل حماية", callback_data="use_filter")]
        ])
...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if broadcast_type == "live_fb":
        if not key.startswith("FB-"):
            ...
            return ConversationHandler.END
        output_base = f"rtmps://live-api-s.facebook.com:443/rtmp/{key}"
    elif broadcast_type == "live_ig":
        output_base = f"rtmps://live-upload.instagram.com:443/rtmp/{key}"
    else:
    ...
    qualities = [
        {"scale": "640:360", "bitrate": "800k", "bufsize": "1600k"},
        {"scale": "854:480", "bitrate": "1400k", "bufsize": "2800k"},
        {"scale": "1280:720", "bitrate": "2500k", "bufsize": "5120k"},
        {"scale": "1920:1080", "bitrate": "4000k", "bufsize": "8000k"},
    ]

    def create_cmd(scale, bitrate, bufsize):
        return [
            "ffmpeg", "-re", "-i", m3u8,
            "-vf", f"{vf},scale={scale}",
            "-c:v", "libx264", "-preset", "veryfast", "-maxrate", bitrate,
            "-bufsize", bufsize, "-g", "50", "-r", "25", "-pix_fmt", "yuv420p",
            "-af", af,
            "-c:a", "aac", "-b:a", "128k", "-ar", "44100", "-ac", "2",
            "-f", "flv", output_base
        ]

    await update.message.reply_text("📡 جاري بدء البث متعدد الجودات (360p - 1080p)...")

    for q in qualities:
        tag = f"{user_id}_{q['scale']}"
        cmd = create_cmd(q["scale"], q["bitrate"], q["bufsize"])
