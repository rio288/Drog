...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎬 تجهيز البث":
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [
                InlineKeyboardButton("Filter", callback_data="use_filter")
            ]
        ])
        await update.message.reply_text(
            "اختر نوع البث أو اختر *Filter* لتفعيل الفلاتر:", 
...
async def add_subscriber(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 2:
        await update.message.reply_text("❗ الاستخدام: /addsub <user_id> <أيام الاشتراك>")
        return
    ...
    try:
        ...
        save_json(USERS_FILE, users)
        await update.message.reply_text(f"✅ تم إعطاء {days} يوم اشتراك للمستخدم {target_id}.")
