...
async def add_subscriber_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ تم إضافة الاشتراك للمستخدم {user_id} لمدة {days} يومًا.")
    try:
        await context.bot.send_message(chat_id=int(user_id), text=f"📢 تم تفعيل اشتراكك لمدة {days} يومًا. استمتع بالبث!")
    except:
        pass
    return ConversationHandler.END

# زر إضافة مشترك من القائمة
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if query.data == "add_subscriber":
        return await add_subscriber_start(update, context)
    await query.answer()
