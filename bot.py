...
LAST_CMDS_FILE = "data/last_cmds.json"
...
def save_last_cmd(user_id, cmd):
    cmds = load_json(LAST_CMDS_FILE)
    cmds[str(user_id)] = cmd
    save_json(LAST_CMDS_FILE, cmds)

def load_last_cmd(user_id):
    cmds = load_json(LAST_CMDS_FILE)
    return cmds.get(str(user_id))
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎬 تجهيز البث":
        ...
        await update.message.reply_text("اختر نوع البث أو اختر *protected* لتفعيل الفلاتر:", reply_markup=keyboard, parse_mode="Markdown")
        ...
        if tag in processes:
            ...
            await update.message.reply_text("⏳ جاري تشغيل آخر بث لك...")

        last_cmd = load_last_cmd(user_id)
        if last_cmd:
            threading.Thread(target=monitor_stream, args=(tag, last_cmd), daemon=True).start()
            await update.message.reply_text("✅ تم إعادة تشغيل البث 🔁")
        else:
            await update.message.reply_text("⚠️ لا يوجد بث سابق لإعادة تشغيله.")
        return ConversationHandler.END
...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_pro:
        cmd = ["ffmpeg", "-re", "-i", link, "-vf", vf, "-c:v", "libx264", "-preset", "veryfast", "-maxrate", "2500k",
               "-bufsize", "5120k", "-g", "50", "-r", "25", "-pix_fmt", "yuv420p", "-af", af,
               "-c:a", "aac", "-b:a", "128k", "-ar", "44100", "-ac", "2", "-f", "flv", output]
    else:
        if broadcast_type == "live_ig":
            cmd = ["ffmpeg", "-re", "-i", link, "-vf", f"scale=1080:1920:force_original_aspect_ratio=decrease,pad=1080:1920:(ow-iw)/2:(oh-ih)/2,{vf}",
                   "-c:v", "libx264", "-preset", "veryfast", "-tune", "zerolatency", "-b:v", "1800k", "-maxrate", "2000k",
                   "-bufsize", "3000k", "-af", af, "-c:a", "aac", "-b:a", "128k", "-f", "flv", "-rtbufsize", "1500M", output]
        else:
            cmd = ["ffmpeg", "-re", "-i", link, "-vf", f"scale=854:-2,{vf}", "-c:v", "libx264", "-preset", "veryfast",
                   "-b:v", "1000k", "-maxrate", "1200k", "-bufsize", "2000k", "-af", af,
                   "-c:a", "aac", "-b:a", "96k", "-f", "flv", output]

    await update.message.reply_text("🚀 جاري بدء البث...")
    increment_daily_stream_count(user_id)
    save_last_cmd(user_id, cmd)
