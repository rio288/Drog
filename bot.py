...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_subscribe_data"):
        try:
            ...
            datetime.fromisoformat(date)  # تحقق من صحة التاريخ
            users = load_json(USERS_FILE)
    ...
    # إعادة تشغيل البث
        ...
        threading.Thread(
            ...
            args=(str(user_id), state["cmd"], user_id, is_subscribed(user_id)),
            daemon=True
        ).start()
...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التحقق من مفتاح البث بناءً على نوع البث
    ...
    # إعداد جودة البث بناءً على الاشتراك
    if is_pro:
        maxrate = "6000k"
        bufsize = "12000k"
        scale_width = 1920  # 1080p عرض
    else:
        maxrate = "2500k"
        bufsize = "5120k"
        scale_width = 1280  # جودة أقل لغير المشتركين

    cmd = [
        "ffmpeg",
        "-re",
        "-i", m3u8,
        "-vf", vf.replace("1280", str(scale_width)),
        "-c:v", "libx264",
        "-preset", "veryfast",
        "-maxrate", maxrate,
        "-bufsize", bufsize,
