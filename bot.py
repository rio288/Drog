...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎬 تجهيز البث":
        ...
        keyboard = InlineKeyboardMarkup([
            ...
            [
                InlineKeyboardButton("BOOST", callback_data="use_filter")
            ]
        ])
        await update.message.reply_text(
            "اختر نوع البث أو اختر *Bost* لتفعيل الفلاتر:",
        ...
        if not is_admin(user_id):
            await update.message.reply_text("❌ هذا الخيار للمشرف فقط.")
            return ConversationHandler.END
        await update.message.reply_text("🔢 أرسل معرف المستخدم (User ID):")
        return ADD_USER_ID
...
async def add_user_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        context.user_data["new_user_id"] = str(user_id)
        await update.message.reply_text("📅 أرسل مدة الاشتراك بالأيام:")
        return ADD_DURATION
    except ValueError:
        await update.message.reply_text("❌ المعرف غير صالح. أرسل رقم فقط.")
        return ConversationHandler.END
...
async def add_duration(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        save_json(USERS_FILE, users)
        await update.message.reply_text(f"✅ تم تفعيل الاشتراك للمستخدم {user_id} حتى {expires.date()}")
    except:
        await update.message.reply_text("❌ حدث خطأ أثناء إضافة الاشتراك.")
    return ConversationHandler.END
