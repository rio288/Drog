...
ADMINS = [8145101051]  # استبدل بمعرفك أو معرفات المسؤولين
USERS_FILE = "data/users.json"
ADMIN_CHAT_ID = -1001234567890  # معرف القناة أو الشات الإداري
...
async def add_subscriber_days(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not days_text.isdigit() or int(days_text) <= 0:
        await update.message.reply_text("❌ عدد الأيام يجب أن يكون رقمًا موجبًا. أرسل عدد صحيح:")
        return ADD_DAYS
    days = int(days_text)
    ...
    expires = datetime.fromisoformat(expires_str) if expires_str else now
    if expires < now:
        expires = now
