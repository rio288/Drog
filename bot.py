...
def monitor_stream(tag, cmd, context: ContextTypes.DEFAULT_TYPE, user_id: int, stream_name: str, is_trial: bool):
    ...
    if is_trial:
        ...
        timer = threading.Timer(600, stop_after_timeout)  # 10 دقائق
        timers[tag] = timer
        timer.start()
...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # فلاتر الحماية إن تم تفعيلها أو المشترك
    if use_filter or is_pro:
        ...
        af_filters = ["asetrate=44100*1.02", "atempo=0.95"]
    else:
        vf_filters = []
        af_filters = []
    ...
    # إعدادات الجودة حسب الاشتراك
    if is_pro:
        # جودة 1080p مع حماية أو بدونها
        vf_scale = "scale=1920:1080:force_original_aspect_ratio=decrease"
        if vf:
            vf_full = vf_scale + "," + vf
        else:
            vf_full = vf_scale
        ...
        cmd = [
            "ffmpeg", "-re", "-i", link,
            "-vf", vf_full,
            "-c:v", "libx264", "-preset", "veryfast", "-maxrate", "4000k",
            "-bufsize", "8000k", "-g", "50", "-r", "25", "-pix_fmt", "yuv420p",
            "-af", af if af else "anull",
            "-c:a", "aac", "-b:a", "128k", "-ar", "44100", "-ac", "2",
            "-f", "flv", output
        ]
    else:
        # جودة منخفضة لغير المشتركين 360p (أو 480p حسب نوع البث)
        if broadcast_type == "live_ig":
            scale_str = "scale=720:1280:force_original_aspect_ratio=decrease"
        else:
            scale_str = "scale=640:360:force_original_aspect_ratio=decrease"

        if vf:
            vf_full = scale_str + "," + vf
        else:
            vf_full = scale_str
