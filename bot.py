...
async def get_m3u8(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not link.endswith(".m3u8"):
        await update.message.reply_text("❌ رابط غير صحيح. يجب أن يكون رابط M3U8.")
        return ConversationHandler.END
...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if broadcast_type == "live_fb":
        if not key.startswith("FB-"):
            await update.message.reply_text("❌ مفتاح Facebook غير صالح. يجب أن يبدأ بـ FB-")
            return ConversationHandler.END
    ...
    # فلترة الفيديو والصوت للحماية (اختياري فقط للمشتركين أو عند التفعيل)
    ...
    # بناء أمر ffmpeg مع دعم أي جودة بدون تغيير، فقط تمرير كما هو مع فلترة اختيارية
    cmd = [
        "ffmpeg", "-re", "-i", link,
        "-vf", vf,
    ...
    tag = str(user_id)  # معرف البث هو معرف المستخدم
