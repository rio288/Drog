...
ADMINS = [8145101051]  # معرفات الأدمن
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎬 تجهيز البث":
        ...
        await update.message.reply_text(
            "اختر نوع البث أو اختر *protected* لتفعيل الحماية:", 
...
async def select_broadcast_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "use_filter":
        ...
        await query.message.reply_text(
            ...
            "سيتم تطبيق الحماية لتجنب الحظر.",
            parse_mode="Markdown"
        )
        return SELECT_BROADCAST_TYPE
...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if use_filter or is_pro:
        vf_filters = ["setpts=PTS/1.005", "boxblur=2:1"]
        af_filters = ["asetrate=44100*1.03", "atempo=0.97"]
    else:
        vf_filters = []
        af_filters = []
