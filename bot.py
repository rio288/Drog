...
SELECT_BROADCAST_TYPE, STREAM_NAME, M3U8_LINK, STREAM_KEY, ADD_SUBSCRIPTION = range(5)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎬 تجهيز البث":
        ...
        if not is_admin(user_id):
            await update.message.reply_text("❌ هذا الخيار للمسؤول فقط.")
            return ConversationHandler.END
        await update.message.reply_text("أرسل الآن معرف المستخدم (ID) الذي تريد تفعيل اشتراكه:")
...
async def add_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        save_json(USERS_FILE, users)
        await update.message.reply_text(f"✅ تم تفعيل الاشتراك للمستخدم `{target_id}` حتى {expires.date()}", parse_mode="Markdown")
    except Exception as e:
        await update.message.reply_text("❌ فشل في إضافة المشترك. تأكد من إدخال رقم صحيح.")
    return ConversationHandler.END
