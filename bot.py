...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if use_filter or is_pro:
        vf_filters = [
            "setpts=PTS*1.02",  # تبطئة الفيديو 2%
            "eq=contrast=1.1:brightness=0.05:saturation=1.2",  # تحسين اللون
            "boxblur=2:1",  # تمويه خفيف
            "noise=alls=10:allf=t+u"  # ضوضاء للحماية
        ]
        af_filters = ["asetrate=44100*1.02", "atempo=0.98"]  # صوت بمزامنة سرعة قريبة جداً
    else:
        vf_filters = []
        af_filters = []
    ...
    # إعداد أمر ffmpeg للبث حسب الاشتراك ونوع البث
    if is_pro:
        cmd = [
            "ffmpeg", "-re", "-i", link,
            "-vf", vf,
