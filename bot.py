...
SELECT_TYPE, STREAM_NAME, M3U8_LINK, STREAM_KEY, ADD_SUBSCRIBE = range(5)
...
def trial_time_left(user_id, stream_name):
    # لكل بث تجربة منفصلة مدتها 10 دقائق (600 ثانية)
    data = load_data()
    user = data.get(str(user_id), {})
    ...
    if not start_time_str:
        # لم يبدأ البث بعد => تجربة متاحة
        return 600  # 10 دقائق بالثواني
...
def run_stream(user_id, stream_name, m3u8, stream_key, broadcast_type, use_filter, is_pro):
    ...
    # فلتر حماية خفيف مع جودة 1080p
    vf_filters = [
        "scale=1920:1080",  # ضبط الدقة 1080p
        "setpts=PTS*1.01",  # تبطئة بسيطة
        "eq=contrast=1.05:brightness=0.02:saturation=1.1",  # تعديل بسيط للألوان
        "boxblur=1:1",  # تمويه خفيف جداً
        "noise=alls=5:allf=t+u"  # ضوضاء بسيطة للحماية
    ]
    ...
    processes[key] = proc
    # تسجيل بدء التجربة إذا غير مشترك
    if not is_pro:
...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إيقاف بث سابق بنفس الاسم إذا موجود
    stop_stream(user_id, stream_name)

    # تشغيل ffmpeg في خيط منفصل
    threading.Thread(
        target=run_stream,
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            STREAM_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_stream_name), stop_stream_name],
