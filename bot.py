...
# إعدادات البوت
TOKEN = os.getenv("TOKEN")  # توكن البوت من متغير البيئة
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = "مشترك ✅" if is_subscribed(user_id) else "غير مشترك ❌"
    ...
    text = (
        ...
        f"معرفك: `{user_id}`\n"
        f"الاسم: {user_name}\n"
        f"الحالة: *{escape_markdown(status, version=2)}*\n\n"
        "اختر من القائمة:\n🎬 تجهيز البث\n⏹ إيقاف البث\n🔁 إعادة تشغيل البث\n📞 تواصل مع الدعم"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🎬 تجهيز البث":
        ...
        await update.message.reply_text("اختر نوع البث أو اختر *Filter* لتفعيل الفلاتر:", reply_markup=keyboard, parse_mode="Markdown")
