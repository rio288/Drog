...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    af_filters = ["asetrate=44100*1.1", "atempo=1.03"] if use_filter or is_pro else []
        ...
        if broadcast_type == "live_ig":
            cmd = [
                ...
                "-vf", f"scale=1080:1920:force_original_aspect_ratio=decrease,"
                       f"pad=1080:1920:(ow-iw)/2:(oh-ih)/2,{vf}",
                "-c:v", "libx264", "-preset", "veryfast", "-tune", "zerolatency",
                "-b:v", "1800k", "-maxrate", "2000k", "-bufsize", "3000k",
                "-af", af, "-c:a", "aac", "-b:a", "128k", "-f", "flv",
