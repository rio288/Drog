...
def monitor_stream(tag, cmd, user_id=None, bot=None, chat_id=None):
    ...
    try:
        for line in proc.stderr:
            if "frame=" in line or "bitrate=" in line:
                if not started and bot and chat_id:
                    bot.send_message(chat_id=chat_id, text="✅ تم بدء البث بنجاح .")
                    started = True
            elif "Connection refused" in line or "Server error" in line:
                if bot and chat_id:
                    bot.send_message(chat_id=chat_id, text="❌ فشل الاتصال بسيرفر البث. تحقق من المفتاح أو الرابط.")
    ...
    proc.wait()
    processes.pop(tag, None)
    if bot and chat_id and not started:
        bot.send_message(chat_id=chat_id, text="⚠️ انتهى البث أو لم يبدأ.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_subscribe_data"):
        try:
            ...
            await update.message.reply_text("❌ خطأ في الصيغة، حاول بالشكل:\n`user_id | 2025-07-01`", parse_mode="Markdown")
    ...
    if text == "🎬 تجهيز البث":
        ...
        if str(user_id) in processes:
            ...
            await update.message.reply_text("⚠️ لا يوجد بث قيد التشغيل.")
        return ConversationHandler.END
...
async def select_broadcast_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "use_filter":
        ...
        await query.message.reply_text(
            "✅ تم تفعيل الحماية من الكوبيرايت\nسيتم تطبيق التعديلات التالية تلقائياً:\n"
            "- ⚡ تقليل التعرف\n- 👻 إخفاء روبوتات\n- 🎄 تجنب الحظر\n\nالآن اختر نوع البث:",
            parse_mode="Markdown"
        )
        return SELECT_BROADCAST_TYPE
...
async def get_stream_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    vf_filters = ["setpts=PTS/1.015", "boxblur=2:1"] if use_filter or is_pro else []
    ...
    if is_pro:
        cmd = [
            "ffmpeg", "-re", "-i", link,
            "-vf", vf, "-c:v", "libx264", "-preset", "veryfast", "-maxrate", "2500k",
        ...
        cmd = [
            ...
            "-af", af, "-c:a", "aac", "-b:a", "128k", "-f", "flv", "-rtbufsize", "1500M",
            output
        ]
    else:
        cmd = [
            "ffmpeg", "-re", "-i", link,
            "-vf", f"scale=854:-2,{vf}", "-c:v", "libx264", "-preset", "veryfast",
            "-b:v", "1000k", "-maxrate", "1200k", "-bufsize", "2000k",
            "-af", af, "-c:a", "aac", "-b:a", "96k", "-f", "flv", output
        ]

    await update.message.reply_text("🎙️ جاري بدء البث...")
    increment_daily_stream_count(user_id)

    threading.Thread(
        target=monitor_stream,
        args=(str(user_id), cmd, user_id, context.bot, update.message.chat_id),
        daemon=True
    ).start()
